...
#!
#!		The arguments of this method are a degree <A>d</A> $\in\mathbb{N}_{\ge 3}$, a radius <A>k</A> $\in\mathbb{N}$, an automorphism <A>aut</A> of $B_{d,k}$, and an involutive compatibility cocycle <A>z</A> of a subgroup of $\mathrm{Aut}(B_{d,k})$ that contains <A>aut</A> (see <Ref Func="InvolutiveCompatibilityCocycle"/>).
#!	</Item>
#! </List>
#!
#! @Arguments d,a
...
#!
#!		The arguments of this method are a degree <A>d</A> $\in\mathbb{N}_{\ge 3}$, and a permutation group <A>F</A> $\le S_{d}$. The kernels output by this method are compatible with <A>F</A> with respect to the standard cocycle (see <Ref Func="InvolutiveCompatibilityCocycle"/>) and can be used in the method <Ref Oper="SIGMA"/>.
#!	</Item>
#!	<Mark>for the arguments <A>d</A>, <A>k</A>, <A>F</A>, <A>z</A></Mark>
#!	<Item>
